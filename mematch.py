...
parser.add_argument('--mem_contrast_weight', default=0.1, type=float)
...
def main_worker(local_rank, args):
    ...
    cocl_dim = cfg.get('cocl_dim', None)
    model = DPT(**{**model_configs[cfg['backbone'].split('_')[-1]], 'nclass': cfg['nclass'], 'cocl_dim': cocl_dim})
    ...
    contras_loss = MemoryBankContrastLoss(
        memory_size=4096,
        feature_dim=cocl_dim,
        temperature=0.1,
        base_temperature=0.07,
        momentum=args.momentum,
        num_classes=cfg.get('nclass')
    )
    ...
    contras_loss = contras_loss.to(device)
    model_ema.eval()
    ...
    valloader = DataLoader(
        ...
        sampler=valsampler, shuffle=False
    )
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        total_loss_mem_contrast = AverageMeter()
        ...
        for i, ((img_x, mask_x),
            ...
            ignore_mask, cutmix_box1, cutmix_box2 = ignore_mask.to(device), cutmix_box1.to(device), cutmix_box2.to(
                device)

            with torch.no_grad():
                pred_u_w, f_w = model_ema(img_u_w)
            ...
            pred_x, f_x = model(img_x)
            out_u_s, f_u_s = model(torch.cat((img_u_s1, img_u_s2)), comp_drop=True)
            pred_u_s1, pred_u_s2 = out_u_s.chunk(2, dim=0)
            f_s1, f_s2 = f_u_s.chunk(2, dim=0)
            ...
            mask_u_w_filtered = mask_u_w.clone()
            mask_u_w_filtered[conf_u_w < args.confidence] = 255

            loss_mem_contrast = contras_loss(f_x, mask_x, f_w, mask_u_w_filtered)
            ...
            mem_contrast_weight = args.mem_contrast_weight

            loss = (loss_x + loss_u_s) / 2.0 + mem_contrast_weight * loss_mem_contrast
            ...
            total_loss_mem_contrast.update(loss_mem_contrast.item())
            ...
            if rank == 0:
                ...
                writer.add_scalar('train/loss_mem_contrast', loss_mem_contrast.item(), iters)
                writer.add_scalar('train/mask_ratio', mask_ratio, iters)

            if (i % (len(trainloader_u) // 8) == 0) and (rank == 0):
                logger.info(
                    'Iters: {:}, LR: {:.7f}, Total loss: {:.3f}, Loss x: {:.3f}, Loss s: {:.3f}, '
                    'Loss mem_contrast: {:.3f}, Mask ratio: {:.3f}'
                    .format(i, optimizer.param_groups[0]['lr'], total_loss.avg, total_loss_x.avg,
                            total_loss_s.avg, total_loss_mem_contrast.avg, total_mask_ratio.avg)
                )
